...
class GithubMemeText extends StatelessWidget {
  ...
  const GithubMemeText({super.key,required this.str,this.fontSize=14,required this.isLight});
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Text(str,style: TextStyle(color: bgColor,fontSize: fontSize,fontWeight: FontWeight.w900,shadows: [
          Shadow(color: bgColor,blurRadius: 1,offset: const Offset(0,0))
        ]),)
        ,
        Text(str,style: TextStyle(color: bgColor,fontSize: fontSize,fontWeight: FontWeight.w900,),)
        ,
        Text(str,style: TextStyle(color: txtColor,fontSize: fontSize),)
      ],
    );
  }
}
